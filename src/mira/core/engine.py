...
class ReviewEngine:
    ...
    async def review_pr(self, pr_url: str) -> ReviewResult:
        ...
        if result.comments:
            ...
            # A completed review with nothing to say deserves a visible
            # "done, all clear" — otherwise silent rounds look like the bot
            # never ran. Only when the review actually reviewed files (early
            # skips return reviewed_files == 0 and already finalize the
            # placeholder with their own reason).
            if (
                self.config.review.chat_updates
                and not self.dry_run
                and not self.shadow
                and result.reviewed_files
            ):
                from mira.core.chatter import review_clean_message

                try:
                    await self.provider.post_comment(pr_info, review_clean_message())
                except Exception as exc:
                    logger.warning("Failed to post review-clear comment: %s", exc)
    ...
    async def _review_diff_internal(
        ...
        # The review proper starts here (all skip checks passed) — say so in
        # the PR. Stdin mode (no provider/pr_info) stays silent.
        if self.config.review.chat_updates and not self.dry_run and not self.shadow:
            pr_info = getattr(self, "_pr_info", None)
            if self.provider is not None and pr_info is not None:
                from mira.core.chatter import review_started_message

                try:
                    await self.provider.post_comment(pr_info, review_started_message())
                except Exception as exc:
                    logger.warning("Failed to post review-start comment: %s", exc)
